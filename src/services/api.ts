...
import type { Connection, Form, FormResponse, Loomer, Project, QueryOptions } from '../types/index.js';
import type { loomersFilter, formsFilter, form_responsesFilter, projectsFilter, loomersOrderBy, formsOrderBy, form_responsesOrderBy, projectsOrderBy } from '../../operations-sdk/schema.js';
...
export async function getLoomers(options: QueryOptions = {}): Promise<Connection<Loomer>> {
  ...
  return {
    edges: response.loomersCollection?.edges.map(({ cursor, node }) => ({
      cursor,
      node: {
        ...node,
        area: node.areas,
...
export async function getForms(options: QueryOptions = {}): Promise<Connection<Form>> {
  ...
  return {
    edges: response.formsCollection?.edges.map(({ cursor, node }) => ({
      cursor,
      node: {
        ...node,
        createdAt: node.created_at,
...
export async function getFormResponses(options: QueryOptions = {}): Promise<Connection<FormResponse>> {
  ...
  return {
    edges: response.form_responsesCollection?.edges.map(({ cursor, node }) => ({
      cursor,
      node: {
        ...node,
        createdAt: node.created_at,
...
export async function getProjects(options: QueryOptions = {}): Promise<Connection<Project>> {
  ...
  return {
    edges: response.projectsCollection?.edges.map(({ cursor, node }) => ({
      cursor,
      node: {
        ...node,
        createdAt: node.created_at,
